...
export var no_clip_speed : float = 4
...
#var state:PlayerState = PlayerState.new(self)
...
func _unhandled_input(event) -> void:
	...
	if state == State.STATE_NOCLIP: return
...
#	state.do(delta)
	match state:
		...
		State.STATE_CLAMBERING_RISE:
			clambering_rise_state(delta)
		State.STATE_CLAMBERING_LEDGE:
			clambering_ledge_state(delta)
		State.STATE_NOCLIP:
			noclip_state(delta)
...
func walk_state(delta):
	...
	if Input.is_action_pressed("lean"):
		state = State.STATE_LEANING; return
	if Input.is_action_pressed("crouch"):
		state = State.STATE_CROUCHING; return
#	if Input.is_action_pressed("crawl"):
#		state = State.STATE_CRAWLING; return
	if Input.is_action_pressed("sneak"):
		_walk(delta, 0.75); return
#	if Input.is_action_just_pressed("noclip"):
#		state = State.STATE_NOCLIP; return
...
func clambering_rise_state(delta):
...
func clambering_ledge_state(delta):
...
func _noclip_walk() -> void:
	...
	var _vel = move_and_slide(move_dir * no_clip_speed)
