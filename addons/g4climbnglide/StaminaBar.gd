...
func _ready():
	if drain_rates.size() > 0:
		for key in drain_rates.keys():
			...
			assert (key in Player.state.keys())
			assert (drain_rates[key] is float)
	pass # Replace with function body.

func change_state(new_state:Player.state):
	current_state = new_state
	if Player.state.keys()[current_state] in drain_rates.keys():
		draining = drain_rates[Player.state.keys()[current_state]]
	else:
		draining = 0
